...
$sql = " SELECT * FROM request WHERE raisedfor = '".$name."' AND rflag = '0'  ";
...
while( $row = mysqli_fetch_assoc($result) ){
			$pid = $row['rid'];
            echo "<tr><td>". $row['raisedby'] ."</td>";
			echo "<td>". $row['atitle'] ."</td>";
			echo "<td>". $row['message'] ."</td>";
			echo "<td><a href='process-request.php?id=". $row['rid']  ." & approve=yes'>Yes </a></td>";
			echo "<td><a href='process-request.php?id=". $row['rid'] ." & approve=no'>No</a></td>";
